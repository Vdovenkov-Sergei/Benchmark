...
1. Предварительно необходимо проверить наличие всех модулей через `pip list` в терминале. Если каких-то библиотек недостаёт, то следует докачать их через `pip install` + название модуля;
> [!NOTE]
